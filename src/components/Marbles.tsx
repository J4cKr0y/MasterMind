...
type Props = {
  onSelectColor: (color: 'red' | 'purple' | 'blue' | 'green' | 'yellow') => void;
...
const Marbles: React.FC<Props> = ({ onSelectColor, selectedColor }) => {
  return (
    <div className="reflect">
      <div className="container flex flex-col justify-around">
        {colors.map((color, index) => (
          <div 
            key={`marble${index}`} 
            data-name={`marble${index}`} 
            className={`${getMarbleClass(color)} ${selectedColor === color ? 'ring-2 ring-blue-500' : ''} cursor-pointer`}
